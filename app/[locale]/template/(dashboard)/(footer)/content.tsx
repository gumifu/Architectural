...
function DashboardPageContent({ dashboard }: DashboardPageData) {
  return (
...
function SalesThisWeek({ dashboard }: DashboardPageData) {
  return (
...
function SalesApexChart({ dashboard }: DashboardPageData) {
...
function NewProductsThisWeek({ dashboard }: DashboardPageData) {
  return (
    <div className="rounded-lg bg-white p-4 shadow sm:p-6 xl:p-8 dark:bg-gray-800">
...
function NewProductsApexChart({ dashboard }: DashboardPageData) {
  const options: ApexCharts.ApexOptions = {
  ...
  return (
    <ApexChart height={305} options={options} series={series} type="bar" />
  );
}

function VisitorsThisWeek({ dashboard }: DashboardPageData) {
  return (
    <div className="rounded-lg bg-white p-4 shadow sm:p-6 xl:p-8 dark:bg-gray-800">
      <div className="flex items-center">
        <div className="shrink-0">
...
function VisitorsApexChart({ dashboard }: DashboardPageData) {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
  ...
  return (
    <ApexChart height={305} options={options} series={series} type="area" />
  );
}

function UserSignupsThisWeek({ dashboard }: DashboardPageData) {
  return (
    <div className="rounded-lg bg-white p-4 shadow sm:p-6 xl:p-8 dark:bg-gray-800">
      <div className="flex items-center">
        <div className="shrink-0">
...
function UserSignupsApexChart({ dashboard }: DashboardPageData) {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
...
function SessionsByCountry({ dashboard }: DashboardPageData) {
  return (
...
function SessionsByCountryMap({ dashboard }: DashboardPageData) {
  const { mode } = useThemeMode();
...
function LatestCustomers({ dashboard }: DashboardPageData) {
  return (
...
function AcquisitionOverview({ dashboard }: DashboardPageData) {
  return (
    <div className="rounded-lg bg-white p-4 shadow sm:p-6 xl:p-8 dark:bg-gray-800">
      <h3 className="text-xl leading-none font-bold text-gray-900 dark:text-white">
...
function AcquisitionApexChart({ dashboard }: DashboardPageData) {
  const { mode } = useThemeMode();
  const isDarkTheme = mode === "dark";
  ...
  return (
    <ApexChart height={305} options={options} series={series} type="donut" />
  );
}

function Transactions({ dashboard }: DashboardPageData) {
                ...
                <TableBody className="bg-white dark:bg-gray-800">
                  {dashboard.transactions.map(
                    ({ transaction, date, amount, status }) => (
                      <TableRow key={`${transaction}-${date}-${amount}`}>
